...
def preprocess_audio_files(arguments, mode='train'):
    ...
    if arguments.data == 'synthetic_sounds':
        ...
        N = 30
        if mode == 'train':
            files_source1 = files_source1[:N]
            files_source2 = files_source2[:N]
    ...
    for i, (fl1, fl2) in enumerate(zip(files_source1, files_source2)):
        wavfl1, fs = lr.load(os.path.join(audio_path, fl1), sr=None)
        wavfl2, fs = lr.load(os.path.join(audio_path, fl2), sr=None)
    ...
    # get the masks 
    masks = []
    for M1, M2 in zip(SPCS1abs, SPCS2abs):
        masks.append( (M1 > M2).astype('float32') )
...
def form_torch_mixture_dataset(MSabs, MSphase, 
    ...
    dataset = MixtureDataset(MSabs, MSphase, SPCS1abs, SPCS2abs, 
                             wavfls1, wavfls2, lens1, lens2, masks)
